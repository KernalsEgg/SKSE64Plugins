...
namespace Skyrim
{
	...
	{
	public:
		enum class EntryPointFunctionDataType : std::uint32_t
		{
			kInvalid              = 0,
			kOneValue             = 1,
			kTwoValue             = 2,
		...
		static_assert(sizeof(EntryPointFunctionDataType) == 0x4);

		// Add
		virtual ~BGSEntryPointFunctionData();                                        // 0
		virtual EntryPointFunctionDataType GetType() const                      = 0; // 1
