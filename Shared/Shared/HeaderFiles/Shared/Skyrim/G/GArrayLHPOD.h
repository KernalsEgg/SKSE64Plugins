...
namespace Skyrim
{
	...
	class GArrayLHPOD :
		public GArrayBase<GArrayData<T, GAllocatorLHPOD<T, StatisticID>, SizePolicy>> // 0
	{
	public:
		using value_type       = T;
		using allocator_type   = GAllocatorLH<T, StatisticID>;
		using size_policy_type = SizePolicy;
		using self_type        = GArrayLHPOD<T, StatisticID, SizePolicy>;
		using base_type        = GArrayBase<GArrayData<T, GAllocatorLHPOD<T, StatisticID>, SizePolicy>>;
