...
namespace Skyrim
{
	...
	public:
		...
		std::uint8_t                                                                      entryPointArgumentCount; // 8
		std::uint8_t                                                                      padding9;                // 9
		std::uint16_t                                                                     paddingA;                // A
		std::uint32_t                                                                     paddingC;                // C
		const char**                                                                      entryPointArgumentNames; // 10
	...
	static_assert(offsetof(BGSEntryPoint, entryPointName) == 0x0);
	static_assert(offsetof(BGSEntryPoint, entryPointArgumentCount) == 0x8);
	static_assert(offsetof(BGSEntryPoint, entryPointArgumentNames) == 0x10);
